...
/*
以下のようにScrollBehaviorを使います
val toolbarHeight = 48.dp　
val toolbarHeightPx = with(LocalDensity.current) { toolbarHeight.roundToPx().toFloat() }
val segmentedButtonScrollBehavior = segmentedButtonScrollBehavior(-toolbarHeightPx)

Box(
   modifier = Modifier
      .nestedScroll(segmentedButtonScrollBehavior.nestedScrollConnection)
      .clipToBounds()
      ){
   SingleChoiceSegmentedButtonRow(
　　   modifier = Modifier
  　　　   .height(toolbarHeight).offset {
        　　   IntOffset(x = 0, y = segmentedButtonScrollBehavior.state.heightOffset.toInt())
     　　   }
...
@OptIn(ExperimentalMaterial3Api::class)
class OriginalEnterAlwaysScrollBehavior(
